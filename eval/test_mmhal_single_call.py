"""
Single test call to OpenRouter GPT-4o for MMHal-Bench scoring.
Makes EXACTLY 1 API call. Estimated cost: < $0.01
"""
import json
import argparse
import openai

# The exact template from MMHal-Bench eval_gpt4.py
...
if __name__ == "__main__":
    ...
    # Count approximate input tokens (rough: 1 token ≈ 4 chars)
    approx_input_tokens = len(input_text) // 4
    print(f"=== SINGLE TEST CALL ===")
    print(f"Question: {record['question']}")
    print(f"Model answer: {record['model_answer']}")
    print(f"Approximate input tokens: {approx_input_tokens}")
    print(f"Estimated cost: ~${approx_input_tokens/1_000_000 * 2.50 + 200/1_000_000 * 10:.4f}")
    print(f"========================")
    print(f"Making 1 API call to GPT-4o via OpenRouter...")
    print()
    ...
    # Project full matrix cost
    cost_per_call = total_cost
    vanilla_cost = cost_per_call * 288
    full_matrix_cost = cost_per_call * 1728
    print(f"\n=== PROJECTED COSTS ===")
    print(f"Cost per call:           ${cost_per_call:.4f}")
    print(f"3 vanilla models (288):  ${vanilla_cost:.2f}")
    print(f"Full matrix (1728):      ${full_matrix_cost:.2f}")
