"""
MMHal-Bench evaluation via OpenRouter GPT-4o.
Based on original eval_gpt4.py from Shengcao1006/MMHal-Bench.
"""
import openai
import argparse
import json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--response', type=str, required=True, help='JSON file with model responses')
    parser.add_argument('--evaluation', type=str, required=True, help='JSON file to save GPT-4o evaluation')
    parser.add_argument('--api-key', type=str, required=True, help='OpenRouter API key')
    parser.add_argument('--model-name', type=str, default='', help='Model name for logging')
    ...
    for i, record in enumerate(records):
        ...
        print(f"[{i+1}/96] {reply[-50:]}", flush=True)
    ...
    for i, resp in enumerate(responses):
        ...
        if len(scores_found) == 1:
            ...
            print(f'Warning: multiple or zero scores found at question {i}')
            scores.append(0)
    ...
    print(f"Hallucination rate: {sum(hallucination)/len(hallucination):.2f}")
    print(f"Per-type scores: {','.join([str(round(sum(s)/len(s), 2)) for s in scores_each])}")
    ...
    summary = {
        ...
        "per_type_scores": [round(sum(s)/len(s), 2) for s in scores_each],
        "all_scores": scores,
        "total_input_tokens": total_input_tokens,
        "total_output_tokens": total_output_tokens,
        "total_cost": round(input_cost + output_cost, 4)
    }
    summary_path = args.evaluation.replace('.json', '_summary.json')
