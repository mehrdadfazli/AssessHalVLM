...
parser.add_argument("--model_type", type=str, default="llava-next", choices=["llava-next", "llava", "instructblip"], help="Model type (fixed to llava-next)")
...
# Derived paths — with resume support
# Check for existing partial JSONL to resume from
existing_jsonls = sorted([
    f for f in os.listdir(model_log_dir)
    if f.startswith(args.model_type) and f.endswith(".jsonl")
]) if os.path.exists(model_log_dir) else []

COMPLETED_IDS = set()
if existing_jsonls:
    # Resume: use the last existing file
    RESPONSES_PATH = os.path.join(model_log_dir, existing_jsonls[-1])
    with open(RESPONSES_PATH, "r") as f:
        for line in f:
            data = json.loads(line)
            COMPLETED_IDS.add(data["image_id"])
    EXP_ID = existing_jsonls[-1].split("_")[-1].replace(".jsonl", "")
    logger.info(f"Resuming: found {len(COMPLETED_IDS)} completed images in {RESPONSES_PATH}")
else:
