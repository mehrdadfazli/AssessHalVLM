...
def _model_call(model, **kwargs):
    """Call model.forward(), dropping any kwargs the model doesn't support."""
    sig = inspect.signature(model.forward)
    supported = set(sig.parameters.keys())
    unsupported = [k for k in kwargs if k not in supported and k != 'kwargs']
...
def generate_AvisC(
    ...
    for _ in range(max_new_tokens):
        ...
        # Masked forward pass
        with torch.no_grad():
            outputs_masked = _model_call(model,
                input_ids=current_input_ids,
