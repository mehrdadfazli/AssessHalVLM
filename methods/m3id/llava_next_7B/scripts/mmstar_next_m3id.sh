...
#SBATCH --job-name=mmstar-next-m3id
#SBATCH --output=/scratch/smansou3/LVLM/lvlm-logs/MMStar/llava_next_M3ID/slurm-%j.log

set -euo pipefail

mkdir -p /scratch/smansou3/LVLM/lvlm-logs/MMStar/llava_next_M3ID
...
LOG_PATH=/scratch/smansou3/LVLM/lvlm-logs/MMStar/llava_next_M3ID
...
echo "python mmstar_eval_llava_next.py \\
...
EXIT_CODE=0
python mmstar_eval_llava_next.py \
    --model-path llava-hf/llava-v1.6-vicuna-7b-hf \
    --jsonl_path /scratch/smansou3/LVLM/datasets/MMStar/mmstar_inputs.jsonl \
    --log_path "$LOG_PATH" \
    --seed 42 \
