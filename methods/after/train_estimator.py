...
if __name__ == '__main__':
    ...
    for need_path, label in (
        (vector_data_path, 'vector_set'),
        (caption_data_path, 'caption_set'),
        (query_data_path, 'query_set'),
    ):
        if not os.path.isfile(need_path):
            feat_dir = os.path.abspath(args.path)
            hint = (
                f"Missing activation file for --{label}.\n"
                f"  Expected: {need_path}\n"
                f"Run get_activations.py with the same --model_name and --dataset_name as that set name, "
                f"and --mode I+Q, T+Q_query, or T+Q_best (must match the split).\n"
                f"Existing .npy files for model prefix {args.model!r} under {feat_dir}:\n"
            )
            try:
                for fn in sorted(os.listdir(feat_dir)):
                    if fn.startswith(args.model) and fn.endswith('.npy'):
                        hint += f"  {fn}\n"
            except OSError:
                pass
