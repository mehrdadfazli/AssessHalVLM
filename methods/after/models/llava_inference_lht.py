    ...
    def evaluate(self, prompt, filepath):
        ...
        with torch.inference_mode():
            output_ids = self.model.generate(
                ...
                max_new_tokens=getattr(self, 'max_new_tokens', 16),
                do_sample=False,
                use_cache=False)
        end_time = time.time()
...
{prompt}\nASSISTANT:"
    ...
    def get_activations_only_text(self, prompt):
        ...
        # Text-only activations should not use image-token processing. Some tokenizers / configs
        # can trigger CUDA asserts when we force image-token handling without images.
