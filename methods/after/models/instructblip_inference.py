...
class InstructBlip(Mllm):
    ...
    def __init__(self, model_name_or_path, **kwargs):
        ...
        # Q-Former has a hard 512 position cap; query tokens are prepended to the text
        # sequence. Truncate long prompts (e.g. MMStar MCQ) and prefer keeping the tail
        # so "Options: ..." stays in context.
        self.processor.tokenizer.truncation_side = "left"
        self.processor.qformer_tokenizer.truncation_side = "left"
        # self.model.to(self.device)

    def _qformer_safe_max_text_tokens(self):
        qcfg = self.model.config.qformer_config
        cap = int(getattr(qcfg, "max_position_embeddings", 512))
        n_query = int(getattr(self.model.config, "num_query_tokens", 32))
        # small margin for BOS/Special tokens inside the Q-Former text span
        return max(cap - n_query - 4, 32)

    def _warn_if_prompt_truncated(self, prompt: str, max_len: int, *, prompt_source: str = None) -> None:
        """Log when a prompt exceeds the Q-Former text budget (left truncation keeps the tail)."""
        tok = getattr(self.processor, "qformer_tokenizer", None) or self.processor.tokenizer
        n_full = len(tok.encode(prompt, add_special_tokens=False))
        if n_full <= max_len:
            return
        dropped = n_full - max_len
        ctx = prompt_source or "unknown"
        logger.warning(
            "InstructBLIP prompt truncated: %d -> %d tokens (%d dropped from start; "
            "Options/Choices tail retained). source=%s prompt_prefix=%r",
            n_full,
            max_len,
            dropped,
            ctx,
    ...
    def _prepare_multimodal_inputs(self, image, prompt, *, prompt_source: str = None):
        max_len = self._qformer_safe_max_text_tokens()
        self._warn_if_prompt_truncated(prompt, max_len, prompt_source=prompt_source)
    ...
    def evaluate(self, prompt, filepath):
        ...
        inputs = self._prepare_multimodal_inputs(image, prompt, prompt_source=filepath)
