"""
Score MMStar JSONL outputs (multiple-choice accuracy).

This copy lives under ``benchmarks/MMStar/`` for sharing with collaborators; it is
kept in sync with ``eval/MMStar/mmstar_eval.py`` in the AFTER repo. Prefer editing
the canonical file and re-copying, or treat this as the standalone kit.
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="JSONL with index, question, answer, response (see benchmarks/MMStar/README.md)",
    )
    parser.add_argument(
        "--summary_csv",
        type=str,
