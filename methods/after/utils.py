...
# Official AMBER query filenames (see AMBER benchmark `data/query/`) vs AFTER category short names.
AMBER_QUERY_FILES = {
    'gen': 'query_generative.json',
    'dis-attribute_sub': 'query_discriminative-attribute.json',
    'dis-existence_sub': 'query_discriminative-existence.json',
    'dis-relation_sub': 'query_discriminative-relation.json',
}


def _resolve_amber_paths(data_path):
    """Support `.../AMBER/{query,image}` or `.../AMBER/data/{query,image}` layouts."""
    for q in (os.path.join(data_path, 'query'), os.path.join(data_path, 'data', 'query')):
        if os.path.isdir(q):
            query_dir = q
            break
    else:
        query_dir = os.path.join(data_path, 'query')
    for im in (os.path.join(data_path, 'image'), os.path.join(data_path, 'data', 'image')):
        if os.path.isdir(im):
            img_base = im
            break
    else:
        img_base = os.path.join(data_path, 'image')
    return query_dir, img_base


def process_data_chair(data_path, opera_results_path=None, seed=42, num_images=500):
    """Load COCO val2014 images for CHAIR evaluation.

    If ``opera_results_path`` is provided, image IDs are taken from that JSONL
    (fixed reproducible set). Otherwise, IDs are sampled from ``COCO_val2014_*.jpg``
    files found under ``data_path``.
    """
    import random as _random
    import re

    image_ids = []
    if opera_results_path:
        with open(opera_results_path, 'r') as f:
            for line in f:
                image_ids.append(json.loads(line)['image_id'])
    else:
        pat = re.compile(r'^COCO_val2014_(\d+)\.jpg$', re.IGNORECASE)
        ...
        if not image_ids:
            raise FileNotFoundError(
                f"No COCO_val2014_*.jpg images found under {data_path}"
            )
    _random.seed(seed)
    _random.shuffle(image_ids)
    image_ids = image_ids[:num_images]
    data_list = []
    for image_id in image_ids:
        new_result = INPUT_DICT.copy()
        new_result['index']   = image_id
        new_result['img_url'] = os.path.join(data_path,
                                    f'COCO_val2014_{image_id:012d}.jpg')
        new_result['prompt']  = "Describe this image."
        new_result['lan']     = 'chair'
        data_list.append(new_result)
    return data_list
...
def process_data_mmstar(
    ...
    """Load MMStar from Hugging Face, cache images locally, build MCQ prompts.

    Matches VLMEvalKit ImageMCQ-style trailing instruction. The HF ``question`` field
    already contains the ``Options: A: ...`` block.
    """
    ...
    for i, row in enumerate(ds):
        ...
        new_result = INPUT_DICT.copy()
        new_result["index"] = idx
        new_result["img_url"] = img_path
        new_result["prompt"] = prompt
        new_result["lan"] = "mmstar"
        new_result["type"] = "choice"
        new_result["answer"] = str(row["answer"]).strip()
...
def process_data_mmhal(data_root, images_root=None):
    """Load MMHal-Bench response_template.json and resolve local images when possible."""
    template_path = os.path.join(data_root, "response_template.json")
    ...
    for i, item in enumerate(items):
        ...
        # Prefer local file (the dataset includes local copies under images/).
        # If the template uses an https URL, basename maps to the local filename.
        basename = os.path.basename(image_src.rstrip("/"))
        local_path = os.path.join(images_root, basename) if basename else ""
        img_url = local_path if (local_path and os.path.isfile(local_path)) else image_src

        new_result = INPUT_DICT.copy()
        new_result["index"] = i
        new_result["img_url"] = img_url
        new_result["prompt"] = item.get("question", "") or ""
        new_result["lan"] = "mmhal"
        new_result["type"] = "free-text"

        # Extra fields for optional analysis / evaluation packaging
        if "gt_answer" in item:
            new_result["gt_answer"] = item.get("gt_answer", "")
        if "question_type" in item:
            new_result["question_type"] = item.get("question_type", "")
        if "question_topic" in item:
            new_result["question_topic"] = item.get("question_topic", "")
        if "image_content" in item:
            new_result["image_content"] = item.get("image_content", [])
        if "image_id" in item:
            new_result["image_id"] = item.get("image_id", "")
...
def process_data_amber(data_path, category):
    data_list = []
    query_dir, img_base = _resolve_amber_paths(data_path)
    fname = AMBER_QUERY_FILES.get(category, f'query_{category}.json')
    file_path = os.path.join(query_dir, fname)
    if not os.path.isfile(file_path):
        file_path = os.path.join(query_dir, f'query_{category}.json')
...
def process_data_pope_activation(dataset_path, model, mode):
    ...
    dataset_root = dataset_dir if os.path.basename(dataset_dir) != 'data' else os.path.dirname(dataset_dir)
    for data in dataset: 
        image_path = os.path.join(dataset_root, 'images', data['image'])
        question = data['text']
...
def process_data_amber_activation(dataset_path, model, mode):
    ...
    dataset_root = dataset_dir if os.path.basename(dataset_dir) != 'data' else os.path.dirname(dataset_dir)
    for data in dataset: 
        image_path = os.path.join(dataset_root, 'image', data['image'])
        question = data['text']
        answer = data['label']
        caption = data['caption']
