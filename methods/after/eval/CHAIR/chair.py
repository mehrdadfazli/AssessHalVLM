...
# Optional bundled corpora (no cluster download): copy e.g. ~/nltk_data/corpora/wordnet*
# into eval/CHAIR/nltk_data/corpora/ — this path is searched first.
_CHAIR_EVAL_DIR = os.path.dirname(os.path.abspath(__file__))
_BUNDLED_NLTK_DATA = os.path.join(_CHAIR_EVAL_DIR, 'nltk_data')
if os.path.isdir(_BUNDLED_NLTK_DATA):
    nltk.data.path.insert(0, _BUNDLED_NLTK_DATA)

# Treebank tokenizer is pure-Python and avoids NLTK 3.9+'s punkt_tab dependency used by word_tokenize.
_TREEBANK_WORD_TOKENIZER = TreebankWordTokenizer()


def _nltk_find(rel_path):
    try:
...
def _has_perceptron_pos_tagger():
    """Avoid calling nltk.pos_tag when corpora are missing (can hang on download attempts)."""
    candidates = (
        'taggers/averaged_perceptron_tagger_eng',
        'taggers/averaged_perceptron_tagger_eng.zip',
    )
    return any(_nltk_find(p) for p in candidates)


_HAS_PERCEPTRON_TAGGER = _has_perceptron_pos_tagger()
...
class CHAIR(object):
    ...
    def caption_to_words(self, caption):
        ...
        # Standard preprocessing (avoid nltk.word_tokenize: NLTK 3.9+ requires punkt_tab,
        # which often fails to download on air-gapped / restricted clusters.)
        words = _TREEBANK_WORD_TOKENIZER.tokenize(caption.lower())
        if _HAS_PERCEPTRON_TAGGER:
            tagged_sent = nltk.pos_tag(words)
        else:
            tagged_sent = [(w, 'NN') for w in words]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cache", type=str, default="./eval/CHAIR/chair.pkl",
    ...
    if args.cache and os.path.exists(args.cache):
        ...
        if not _HAS_PERCEPTRON_TAGGER:
            print(
                "[chair.py] NLTK POS tagger not found; using NN for all tokens "
                "(no network; scores remain usable).",
                file=sys.stderr,
            )
