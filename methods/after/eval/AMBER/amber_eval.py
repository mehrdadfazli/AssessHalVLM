...
def get_args():
    ...
    parser.add_argument('--validate_dataset', type=str, default='AMBER', help="specifies the path to the data")
    parser.add_argument('--save_dir', type=str, default='./scores')
    parser.add_argument('--summary_dir', type=str, default='./summaries')
    parser.add_argument('--data_dir', type=str, default='./results')
    parser.add_argument('--device', type=str, default='0')
    parser.add_argument('--dimensions', type=str, default='all', help="Space-separated: gen, dis_sub, dis-attribute_sub, ... Use gen for Table 1 CHAIR/Hal/Cover.")
    parser.add_argument('--subfix', type=str, default='')
    parser.add_argument("--word_association", type=str, default=os.path.join(_EVAL_DIR, 'relation.json'))
    parser.add_argument("--safe_words", type=str, default=os.path.join(_EVAL_DIR, 'safe_words.txt'))
    # parser.add_argument("--inference_data", type=str)
    parser.add_argument("--annotation", type=str, default=os.path.join(_EVAL_DIR, 'annotations.json'))
    parser.add_argument("--metrics", type=str, default=os.path.join(_EVAL_DIR, 'metrics.txt'))
...
def main(args):
    ...
    if args.dimensions[0] == 'all':
        # Generative (Table 1 CHAIR/Hal/Cover) + merged discriminative from sub-queries
        args.dimensions = ['gen', 'dis_sub']
    for dim in args.dimensions:
        metrics = init(args)
        dimension = {'g': False,'de': False, 'da': False, 'dr': False}
        if dim == 'gen':
            dimension['g'] = True
        elif dim == 'dis' or dim == 'dis_sub':
            dimension['de'] = True
            dimension['da'] = True
            dimension['dr'] = True
        else:
        ...
        result_dir = os.path.join(args.data_dir, args.validate_dataset)
        datapath = os.path.join(result_dir, f'{args.model}_{dim}{args.subfix}.jsonl')
        print(f'Scoring on {dim} of {args.validate_dataset} with Acc/Prec/...')
        if dim == 'dis_sub' and not os.path.isfile(datapath):
            subs = ['dis-attribute_sub', 'dis-existence_sub', 'dis-relation_sub']
            merged = []
            for sub in subs:
                p = os.path.join(result_dir, f'{args.model}_{sub}{args.subfix}.jsonl')
                if os.path.isfile(p):
                    merged.extend([obj for obj in jsonlines.open(p, 'r')])
            if not merged:
                raise FileNotFoundError(
                    f"No discriminative results found: missing {datapath} and no per-sub files "
                    f"{args.model}_dis-{{attribute|existence|relation}}_sub{args.subfix}.jsonl under {result_dir}"
                )
            inference_data = sorted(merged, key=lambda x: x['index'])
        else:
            if not os.path.isfile(datapath):
                raise FileNotFoundError(f"Inference file not found: {datapath}")
            inference_data = [obj for obj in jsonlines.open(datapath, 'r')]
        # inference_data = json.load(open(datapath, 'r', encoding='utf-8'))

        for i in tqdm(range(len(inference_data))):
            id = inference_data[i]['index']
        ...
        if dimension['g']:
            ...
            print("Hal:\t\t", Ha_p)
            print("Cog:\t\t", Ha, "\n")
            result = pd.DataFrame({'Category':'Generative', 'CHAIR/Accuracy':CHAIR, 'Cover/Precision': Cover, 'Hal/Recall': Ha_p, 'Cog/F1': Ha},index=[0])
        ...
        if dimension['de'] and dimension['da'] and dimension['dr']:
            ...
            print("Recall:\t\t", Recall)
            print("F1:\t\t", F1, "\n")
            result = pd.DataFrame({'Category':'Discriminative', 'CHAIR/Accuracy':Accuracy, 'Cover/Precision': Precision, 'Hal/Recall': Recall, 'Cog/F1': F1},index=[0])
            results.append(result)
